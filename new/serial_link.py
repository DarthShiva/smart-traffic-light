...
Protocol (must match traffic_control_arduino.ino exactly):
    9600 baud, 8N1
    one line per update: <North><East><South><West>:<counter>\\n
    each state character is 'R' or 'G' - the firmware coerces anything
    else to red, so this module refuses to send a string that is not
    exactly NUM_LANES characters of R/G rather than letting the hardware
    silently disagree with the dashboard.

    <counter> is a rolling, strictly-increasing non-negative integer. The
    firmware rejects any line whose counter is <= the last counter it
    accepted, so a captured-and-replayed line (or anything not produced
    by this module) is ignored instead of silently re-applied. The
    firmware's counter resets whenever the board itself resets/reboots -
    which also happens whenever we open the serial port - so this module
    resets its own counter back to 0 on every successful connect(), and
    every write increments it. Two ArduinoLink instances (or a
    disconnect/reconnect cycle) therefore always stay in sync with the
    board's own reset behaviour.
...
class ArduinoLink:
    ...
    def __init__(self, baudrate: int = SERIAL_BAUDRATE, timeout: float = SERIAL_TIMEOUT_SEC):
        ...
        self._counter = 0  # next rolling counter to send; mirrors the board's own reset
        self.last_error: Optional[str] = None
        self.last_sent: Optional[str] = None
        self.last_sent_at: Optional[float] = None
        self.last_sent_counter: Optional[int] = None
    ...
    def status(self) -> dict:
        ...
        return {
            ...
            "last_sent_counter": self.last_sent_counter,
        ...
        with self._lock:
            if self._conn is conn:
                try:
                    ...
                    pass
                # The firmware's own rolling counter resets to -1 whenever
                # the board reboots, which just happened. Reset ours to 0
                # (its lowest acceptable value) so the very next line we
                # send is guaranteed to be accepted rather than rejected
                # as stale relative to whatever we'd counted up to before.
                self._counter = 0
    ...
    def _close_locked(self) -> None:
        ...
        # Whatever we counted up to belonged to a session the board no
        # longer remembers once we reconnect (connect() reboots it), so
        # don't let a stale high counter linger between sessions.
        self._counter = 0

    # ------------------------------------------------------------ write --
    def send_state(self, state_string: str) -> bool:
        """Write one R/G line, tagged with the next rolling counter.
        Returns True only if the bytes really went out. A failed write
        drops the link so the UI stops claiming the board is connected."""
        ...
        with self._lock:
            ...
            line = f"{state_string}:{counter}"
            try:
                conn.write((line + "\n").encode("ascii"))
                conn.flush()
                # Only advance the counter once the write actually
                # succeeded - a failed write must not burn a counter
                # value the board never saw.
                self._counter += 1
